...
初めまして。
大学3年生のころから、42tokyoでコンピュータサイエンスを学び始めました。
C言語を用いた、システムプログラミングから始まり、シェルやレイトレーシングプログラムの再実装など、幅広い技術を習得しました。
このような経験は、パフォーマンスと効率性が重視されるAI開発において独自の視点を与えてくれます。現在はその強みを活かしAIエージェント開発に取り組んでいます。

💡 現在の取り組み：AIセールスエージェントの開発

顧客との会話をリアルタイムで分析し、最適な提案を生成するAIエージェントの開発に取り組んでいます。営業担当者のパフォーマンスを最大化し、成約率を向上させることが現在の目標です。

今後の展望：RAGによるシステムの信頼性強化
将来的には、研修で習得した**RAG Retrieval-Augmented Generation**の知識を応用し、AIエージェントを社内の製品情報やドキュメントと連携させることで、応答の正確性と信頼性を飛躍的に向上させることを目指しています。これにより、ハルシネーション（事実に基づかない情報の生成）を抑制し、常に事実に基づいた応答ができるシステムの実現を計画しています。

🚀 42 Tokyoでの学びと実績
<summary><strong>42 Tokyoとは？</strong></summary>

フランス発のエンジニア養成機関です。授業料は完全無料で、従来の「先生」や「授業」は存在しません。すべての学習は、学生同士が協力して課題を解決するピアラーニングと、実践的なプロジェクトベースのカリキュラムで構成されています。このユニークな環境で、自走力と現場で通用する問題解決能力を徹底的に鍛えました。


42での経験は、私の技術的な基盤を形成しています。以下に私の42tokyoでの学習遍歴を示します。
...
### 2. **miniRT** [![C](https://img.shields.io/badge/C-100%25-success)]()
**Key Features**:

📐 Vector and matrix math library from scratch

Intersection calculations for spheres, planes, and cylinders

💡 Phong reflection model for realistic lighting (ambient, diffuse, specular)

🎥 Camera transformations and scene file parsing (.rt)

3DのCG画像を生成するレイトレーシングエンジンです。C言語とグラフィックスライブラリ（minilibx）のみを使用し、ゼロから実装しました。このプロジェクトの目的は、3D空間における物体、光、カメラの相互作用を数学的にモデル化し、ベクトル演算や幾何学の知識を実践的なプログラムに落とし込むスキルを習得することでした。

シーン記述ファイル（.rt）を読み込み、そこに定義された物体や光源情報を基に、カメラからスクリーンの一点（ピクセル）に向けてレイ（光線）を飛ばします。レイと物体の交差判定を行い、最も近い交点の色を照明モデルに基づいて計算することで、最終的な2D画像を生成します。特に、法線ベクトルや反射光の計算など、リアルな質感を表現するための数学的知識が求められました。
